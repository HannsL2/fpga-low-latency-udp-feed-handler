...
if {$argc != 1} {
    error "Usage: vivado -mode batch -source scripts/run_directed_simulation.tcl -tclargs <tb_ethernet_parser|tb_ipv4_parser>"
}

set testbench [lindex $argv 0]
set supported_testbenches [list tb_ethernet_parser tb_ipv4_parser]

if {[lsearch -exact $supported_testbenches $testbench] < 0} {
    error "Unsupported testbench '$testbench'. Choose tb_ethernet_parser or tb_ipv4_parser."
}
